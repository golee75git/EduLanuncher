...
브랜치: `main`  
최신 커밋 시점의 앱 버전: `0.1.93-20260924`
...
- GitHub Release에 `0.1.93` 설치파일 업로드
...
- 사이트 칸에서 이 PC Edge·Chrome 북마크와 Windows `.url` 목록. Windows `.url`을 넣으면 그 파일 아이콘을 남김. 탐색기 보내기 → 교육업무 런처로 `.url`을 넣음. `.url` 기본 연결은 바꾸지 않음
