...
브랜치: `main`  
최신 커밋 시점의 앱 버전: `0.1.71-20260922`
...
- GitHub Release에 `0.1.71` 설치파일 업로드
- 패널 가로 약 560px 이상이면 홈·모두(2열 설정) 목록 3열. FavoriteGrid 1·2·3열
...
- 처음 설치에는 사이트·프로그램·파일·폴더가 비어 있음. 설정에 교육 바로가기 추가 단추 없음. 업무도구는 Network·CCTV·주소 무늬. 컴퓨터도구·업무자료는 앱에 있음. 이미 쓰는 PC 목록은 그대로. 할 일 샘플도 넣지 않음. 홈 자주 사용하는 도구의 사이트·프로그램·파일·폴더는 비어 있으면 끌어놓기나 + 안내가 나옴. 종류 이름은 구역 제목보다 한 단계 작게(13px). 종류마다 2열로 최대 6개까지 보이고, 더 있으면 모두에서 봄
