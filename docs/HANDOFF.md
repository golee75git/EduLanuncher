...
브랜치: `main`  
최신 커밋 시점의 앱 버전: `0.1.62-20260921`
...
- 홈 아래 메모. 크게는 런처 옆에 창을 연다. 이동으로 창을 옮긴다. 패널을 숨기면 같이 숨고, 다시 열면 옮긴 자리에 있다. 제목 줄 X로 닫으면 다음 크게는 다시 옆에서 연다. 저장소는 패널만 연다.
- Pack vs 이 PC 표시, 홈 그룹 3개+모두(바로가기만)
- 다른 PC용 백업 JSON (설정 화면)
- git 연결, Cloudflare Workers 정적 사이트 (`website/`)
- GitHub Release에 `0.1.62` 설치파일 업로드
...
- 처음 설치에는 사이트·프로그램·파일·폴더가 비어 있음. 설정에 교육 바로가기 추가 단추 없음. 업무도구는 Network·CCTV·주소 무늬. 컴퓨터도구·업무자료는 앱에 있음. 이미 쓰는 PC 목록은 그대로. 할 일 샘플도 넣지 않음. 홈 자주 사용하는 도구의 사이트·프로그램·파일·폴더는 비어 있으면 끌어놓기나 + 안내가 나옴. 종류마다 2열로 최대 6개까지 보이고, 더 있으면 모두에서 봄
