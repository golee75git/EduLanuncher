...
브랜치: `main`  
최신 커밋 시점의 앱 버전: `0.1.49-20260920`
...
- GitHub Release에 `0.1.49` 설치파일 업로드
...
- 홈 검색·업무자료 검색 결과 위에 업무 그림(자체 SVG). 크게 보기는 런처와 같은 높이로 옆에 붙음. 그래프 라이브러리 없음. 별도 창은 패널과 같은 index.html을 열고 저장소는 열지 않음. 이미 있는 창은 지우고 다시 연다
