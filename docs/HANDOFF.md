...
브랜치: `main`  
최신 커밋 시점의 앱 버전: `0.1.58-20260921`
...
- GitHub Release에 `0.1.58` 설치파일 업로드
...
- 처음 설치에는 사이트·프로그램·파일·폴더가 비어 있음. 설정에 교육 바로가기 추가 단추 없음. 업무도구는 Network·CCTV·주소 무늬. 컴퓨터도구·업무자료는 앱에 있음. 이미 쓰는 PC 목록은 그대로. 할 일 샘플도 넣지 않음
- 설정의 바탕화면 미니 창(기본 끔). 켜면 왼쪽 아래에 오늘 남은 할 일 개수와 런처 열기만 있는 작은 창. 패널을 숨겨도 남음. 제목 줄 X는 그 창만 숨김. Windows 위젯 보드·바탕화면 아이콘 뒤 붙이기 아님. 홈을 먼저 띄운 뒤 창을 만들고, 할 일 숫자는 이벤트로만 받음
