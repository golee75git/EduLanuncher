...
브랜치: `main`  
최신 커밋 시점의 앱 버전: `0.1.94-20260924`
...
- GitHub Release에 `0.1.94` 설치파일 업로드
...
- 사이트 칸에서 이 PC Edge·Chrome 북마크와 Windows `.url` 목록. Windows `.url`을 넣으면 그 파일 아이콘을 남김. 탐색기 보내기 → 교육업무 런처로 `.url`을 넣음. `.url` 기본 연결은 바꾸지 않음. `.url` 본문의 `IconFile`이 이 PC 안 `.ico/.exe/.dll`이면 그 그림을 우선 쓰고, 경로 없이 넘어오는 가상 파일 `.url`(IE 즐겨찾기 등)도 이름·본문을 읽어 같은 방식으로 처리(`drop_target.rs`, `shell_icon.rs`). 원격 아이콘 주소는 받지 않음
