...
- 업무도구 칸 「컴퓨터도구」: 이 PC `System32`의 고정 파일만 열어 이름 바꾸기·제어판·시스템 정보 등 Windows 설정 화면을 연다. 이 PC IP 주소에서 사설·공인 IP만 본다. 홈 칸을 늘리지 않음
- 바로가기 카드 별로 홈 표시를 바꾸고, 삭제는 확인 후 목록에서만 뺀다. 관리자가 필요한 프로그램은 셸로 연다
