...
Implementation:
`memo-pad` 창을 패널 옆에서 연다. 처음 열 때만 옆으로 옮기며 조금 더 넓게 한다. 이동 단추는 이 창을 끈다. 패널을 숨기면 창도 숨기고, 다시 열면 창 위치는 그대로 둔다. 제목 줄 X로 닫으면 다음 크게는 다시 옆에서 연다. 글은 패널이 저장하고, 창은 짧게 받아 보여 준다. 창은 저장 파일을 열지 않는다. 여러 장·노란 스티커·항상 위는 없다
