...
export function NoticePackPick({ pack, sitePack, onClose, onAdded }: NoticePackPickProps) {
  ...
  const existingTools = useToolStore((state) => state.tools);
  const addFromPack = useNoticeStore((state) => state.addFromPack);
  const already = useMemo(() => new Set(existing.map((item) => item.id)), [existing]);
  const alreadySites = useMemo(
    () => new Set(existingTools.map((item) => item.id)),
    [existingTools],
  );
  const sites = sitePack?.tools ?? [];
  const fresh = pack.notices.filter((item) => !already.has(item.id));
  const freshSites = sites.filter((item) => !alreadySites.has(item.id));
  const [picked, setPicked] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(fresh.map((item) => [item.id, true])),
  );
  const [sitePicked, setSitePicked] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(freshSites.map((item) => [item.id, true])),
  );
  const [busy, setBusy] = useState(false);

  const selectedIds = fresh.filter((item) => picked[item.id]).map((item) => item.id);
  const selectedSiteIds = freshSites.filter((item) => sitePicked[item.id]).map((item) => item.id);
  const canApply = selectedIds.length > 0 || selectedSiteIds.length > 0;
    ...
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-desk/40 p-4 backdrop-blur-sm">
      <div className="card-surface flex max-h-[90%] w-full max-w-sm flex-col p-4 shadow-pop">
        <h3 className="text-base font-semibold text-desk">
          {sites.length > 0 ? "공지·사이트 Pack에서 고르기" : "공지 Pack에서 고르기"}
        </h3>
        <p className="mt-2 text-sm leading-6 text-quiet">
          {pack.name}
          {sites.length > 0 ? "" : ` · ${NOTICE_KIND_LABEL[pack.kind]}`}. 이미 있는 공지·사이트는
          건너뜁니다. 고른 항목만 넣습니다.
        </p>
        <div className="mt-3 min-h-0 flex-1 space-y-3 overflow-y-auto">
          {pack.notices.length > 0 ? (
            <section className="space-y-1">
              <h4 className="text-xs font-medium text-quiet">공지</h4>
          ...
          {sites.length > 0 ? (
            <section className="space-y-1">
              <h4 className="text-xs font-medium text-quiet">사이트</h4>
              <ul className="space-y-1">
                {sites.map((item) => {
                  const kept = alreadySites.has(item.id);
                  return (
                    <li key={item.id} className="flex items-start gap-2 rounded-lg px-1 py-1">
                      <input
                        type="checkbox"
                        className="mt-1 h-4 w-4"
                        disabled={kept}
                        checked={kept ? true : Boolean(sitePicked[item.id])}
                        onChange={(event) =>
            ...
            onClick={() => {
              void (async () => {
                ...
                try {
                  ...
                  if (sitePack && selectedSiteIds.length > 0) {
                    const chosen = sitePack.tools.filter((item) =>
                      selectedSiteIds.includes(item.id),
                    );
                    const result = await useToolStore.getState().applyLauncherPack({
                      ...sitePack,
                      tools: chosen,
                    });
