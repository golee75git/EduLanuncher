      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        ...
        <SettingsCard title="보기">
          <p className="text-xs leading-5 text-quiet">
            서류·밝은 화면·어두운 화면은 패널 안 색만 바꿉니다. 이전스킨은 카드형 홈 이전 배치이고 색은 서류입니다. 창 테두리는 Windows가 그립니다.
          </p>
          <label className="block text-sm text-desk">
            기본 스킨
            <select
          ...
          <p className="text-xs leading-5 text-quiet">
            ...
            자주 사용하는 도구는 최소 너비에서 1열, 기본 창에서 2열, 그 너비에서 3열입니다.
          </p>
          <Toggle
            label="홈 검색 빈 구역 숨김"
            checked={settings.hideEmptySearchGroups}
            onChange={(hideEmptySearchGroups) => void update({ hideEmptySearchGroups })}
          />
          <p className="text-xs leading-5 text-quiet">
        ...
        <SettingsCard title="업무자료">
          <p className="text-xs leading-5 text-quiet">
            홈 검색 또는 업무도구의 업무자료에서 관련 업무를 찾습니다. 편람 분류는 업무자료와 따로 둡니다. 검색 결과 위에 업무 그림이 나오고, 크게 보기에서 런처 옆을 조금 더 넓게 펼칩니다. 그림은 그 창을 채웁니다. 자세한 업무에서 뒤로 가면 방금 검색한 화면이 다시 나옵니다. 업무지도 탭은 목록입니다. 교육행정전자서명(EPKI) 인증서는 같은 JSON에서 검색·목록·상세가 이어집니다.
          </p>
          {import.meta.env.DEV ? (
          ...
          {newerRelease ? (
            <div className="mt-2 space-y-2">
              <p className="text-sm leading-6 text-desk">
                이 PC 버전 {APP_CONFIG.version} · 새 버전 {newerRelease}가 있습니다. 소개 사이트에서 받아 이
        ...
        <SettingsCard title="사용 방법">
          ...
          <ul className="mt-2 list-disc space-y-1.5 pl-4 text-xs leading-5 text-quiet">
            ...
            <li>설정의 보기에서 서류·밝은 화면·어두운 화면·이전스킨과 모두 목록 1열·2열을 고릅니다. 이전스킨은 카드형 홈 이전 배치이고 색은 서류입니다. 홈의 자주 사용하는 도구는 창을 최소 너비로 줄이면 1열, 기본 창에서는 2열, 가로로 넓히면(약 560px 이상) 3열입니다. 이전스킨의 홈 목록은 좁으면 2열, 넓으면 3열입니다. 모두 목록은 1열 설정을 유지하고, 2열 설정일 때 같은 너비에서 3열이 됩니다. 홈 검색 빈 구역 숨김은 기본이 꺼져 있어 지금처럼 빈 안내를 둡니다. 켜면 일치가 없는 구역만 숨기고 순서는 그대로입니다.</li>
            <li>트레이, 작업 표시줄 아이콘 또는 Ctrl+Alt+E로 패널을 엽니다. 홈 검색창 안내는 사이트·업무·파일·폴더·컴퓨터도구·PC 문제입니다. 홈에서 Ctrl+K를 누르면 검색창으로 이동합니다. 검색창에는 그 키 글자를 그리지 않습니다. 창을 닫아도 작업 표시줄 아이콘은 남고, 아이콘을 누르면 설정한 자리(기본은 오른쪽 아래)에 다시 열립니다. 컴퓨터를 켜 두면 아이콘이 있습니다. 완전히 끄려면 트레이에서 종료합니다. 창 모서리를 끌어 크기를 바꿀 수 있고, 바꾼 크기는 이 PC에 남습니다. 처음 설치는 440×650입니다.</li>
            <li>설정의 공지·사이트 Pack에서 나눌 공지·사이트를 고른 뒤 한 파일로 저장합니다. 받는 쪽에서 공지와 사이트를 고릅니다. 할 일·메모·설정은 넣지 않으며, 다른 PC로 옮기기(백업)와는 다릅니다.</li>
            <li>홈 위 인사는 5시–11시 29분에 좋은 아침이에요, 11시 30분–12시 59분에 즐거운 점심시간이에요, 13시–16시 59분에 좋은 오후예요, 17시–다음날 4시 59분에 좋은 저녁이에요입니다.</li>
            <li>처음 설치에는 사이트·프로그램·파일·폴더와 할 일이 비어 있습니다. 그때만 자주 사용하는 도구 아래에 “사이트·프로그램·파일·폴더를 패널에 끌어 놓으면 추가됩니다.”가 나옵니다. 그중 하나라도 있으면 이 문장은 숨깁니다. 오른쪽 위 날짜는 9월26일(토)처럼 요일을 붙입니다. 홈의 자주 사용하는 도구에 끌어놓기나 + 안내가 나옵니다. 종류 이름(사이트·프로그램 등)은 구역 제목보다 한 단계 작게 두고, 모두·즐겨찾기 등 단추 글자는 그대로입니다. 종류마다 2열로 최대 6개까지 보이고, 더 있으면 모두에서 봅니다. + 또는 끌어놓기로 넣고, 설정에서 Pack으로 나눠 줍니다. 이미 쓰는 PC의 바로가기·할 일 목록은 그대로입니다. 업무도구에는 QR코드 넣기가 있고, 컴퓨터도구와 업무자료는 앱에 있습니다. Network·CCTV는 첫 설치에 넣지 않으며, 담당자용 Pack을 적용한 PC에만 목록에 생깁니다.</li>
            ...
            <li>홈 아래 메모의 크게 보기는 런처 옆에 창을 엽니다. 제목 줄로 옮기고, 모서리로 크기를 바꿉니다. 바꾼 크기는 이 PC에 남습니다. 패널을 숨기면 같이 숨고, 다시 열면 옮긴 자리·크기가 있습니다. 제목 줄 X로 닫으면 다음 크게는 다시 옆에서 엽니다.</li>
            <li>홈 검색 결과는 관련 도구, 최근 사용, 이 PC 폴더, 학교, PC 문제 해결, 관련 업무 순입니다. 관련 도구 줄마다 사이트·프로그램·파일·폴더·업무도구가 보입니다. PC 문제 해결은 학교와 관련 업무 사이에 최대 5줄이 나오고, 더 보기에서 분류·검색 목록을 엽니다. 관련 업무와 섞지 않습니다. 설정의 보기에서 빈 구역 숨김을 켜면 일치가 없는 구역은 빼고, 모두 없으면 한 줄만 남깁니다. 이 PC 폴더는 한 글자일 때 두 글자 안내를 남깁니다. 관련 업무 위에 업무 그림이 있고, 크게 보기에서 런처 옆을 조금 더 넓게 펼칩니다. 그림은 창을 채웁니다. 확대·축소 단추와 마우스 휠로 크기를 바꾸고, 끌어서 옮깁니다. 상자를 누르면 패널에 자세한 업무가 열립니다. 그 창은 제목 줄 X로 닫습니다. 패널을 숨기면 같이 숨습니다. 프로그램은 트레이 종료로 끕니다. 관련 업무를 연 뒤 위쪽 뒤로 또는 오른쪽 아래 이전화면을 누르면 검색어와 그림·목록이 그대로 있습니다. 처음 화면을 누르면 빈 홈으로 갑니다.</li>
            <li>홈 검색에 두 글자 이상 적으면 맨 아래에 이 PC 폴더가 나옵니다. 바탕화면·문서·다운로드의 이름만 보고, 내용은 읽지 않습니다. 더 보기에서 사진·음악·동영상도 고를 수 있습니다.</li>
            <li>홈 업무도구 줄의 단축키, 업무자료, 편람 분류, 컴퓨터도구 글자 앞에 작은 그림이 있습니다. 누르는 화면은 이전과 같습니다.</li>
            <li>업무도구 칸의 단축키에서 Windows·문서 단축키 20개를 봅니다. 탐색기·화면 잠금·설정·작업 관리자·화면 캡처는 실행으로 같은 화면이나 프로그램을 엽니다. 키를 대신 누르지는 않습니다.</li>
            <li>업무도구 칸의 편람 분류는 편람 흐름 자료 19개 분류와 그 업무 그림입니다. 여러 업무에 같은 문장이 들어간 항목은 그 공통 그림을 그리지 않습니다. 분류 이름과 같은 기존 업무 순서가 하나뿐이면 그 제목으로 그림을 그립니다. 순서가 둘 이상이거나 없으면 순서를 보여 주지 않습니다. 기존 업무자료와 번호가 같아도 합치지 않습니다. 그림이 안 열리면 같은 화면의 목록을 씁니다. 쪽수는 출처 표기만입니다.</li>
            <li>인증서를 검색하면 안내 문장에 https://www.epki.go.kr/ 이 있고, 그 주소가 있는 자료는 원문 보기로 엽니다.</li>
            <li>업무도구 칸의 업무자료에서 검색결과와 업무지도를 고릅니다. 검색결과 위에도 업무 그림이 있습니다. 관련 업무를 연 뒤 뒤로 가면 방금 검색한 화면이 다시 나옵니다. 업무지도 항목을 누르면 자세한 업무가 열립니다. 인증서(EPKI)는 같은 목록에서 발급·갱신·재발급·백업을 찾습니다. JSON에 주소가 있는 자료만 바깥에서 엽니다. 자료에는 강원 기준, 전국 공통, 타 교육청 참고, 지역 확인 필요가 표시됩니다. 타 교육청 값은 강원 기준으로 바꾸지 않습니다. 학교알리미·나라장터·교육시설 통합정보망은 제목이 일치하면 공식 누리집을 엽니다. 업무 자세히 보기에는 편람에서 확인한 상황·처리 순서·확인할 사항·서류와 인쇄 쪽수가 있고, 확인하지 않은 금액·기한은 적지 않습니다. 일부 업무는 아이콘이 있는 흐름 상자와 조건 버튼을 눌러 볼 수 있고, 그 문장은 원문 확인 전입니다. 통계 숫자가 없는 업무에는 차트를 그리지 않습니다. 주소가 있는 자료만 원문 보기가 나옵니다.</li>
            <li>업무도구 칸의 컴퓨터도구는 이 PC Windows 설정 화면을 열거나, 사설·공인 IP와 익스플로러 설정 복원 확인 화면을 엽니다. 소리·인터넷 옵션·작업 관리자 화면도 엽니다. 「이 PC 폴더 찾기」는 이 사용자 폴더의 이름만 찾습니다. 「PC 문제 해결」은 인터넷·프린터·Windows 증상 안내입니다. 질문에 답하면 다음 안내가 나옵니다. 시스템 명령을 대신 실행하지 않습니다. Microsoft 공식 주소만 브라우저로 엽니다. 검색어는 파일에 넣지 않습니다.</li>
